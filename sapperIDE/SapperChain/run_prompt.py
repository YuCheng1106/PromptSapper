import openai
from SapperChain import LLMConfigurator
import re
...
def getPromptParams(prompt_template):
    ...
    if re.search(r"{{\w+}}", prompt_template):
        ...
        for i in range(len(paraNames)):
            if(paraNames[i][2:-2] not in result):
                result.append(paraNames[i][2:-2])
    return result
def run_Function(promptvalue, prenunits ,model,OpenAIKey,debugvalue):
    ...
    try:
        ...
        if(model["engine"].replace(" ","")=="gpt-3.5-turbo" or model["engine"].replace(" ","")=="gpt-3.5-turbo-16k"):
